...
const uint8_t zh12x12[][28] = {
    ...
    /* 18 内 */ {0xe5,0x86,0x85,0x00,0x00,0xfc,0x04,0x84,0x44,0x3f,0x24,0x44,0x84,0x04,0xfc,0x00,0x00,0x07,0x00,0x00,0x00,0x00,0x00,0x00,0x04,0x04,0x07,0x00,},
    /* 19 部 */ {0xe9,0x83,0xa8,0x00,0x12,0xd6,0x5a,0x53,0x5a,0xd6,0x12,0xff,0x11,0x29,0xc7,0x00,0x00,0x07,0x02,0x02,0x02,0x07,0x00,0x07,0x01,0x02,0x01,0x00,},
    /* 20 气 */ {0xe6,0xb0,0x94,0x00,0x10,0x08,0x24,0x2b,0x2a,0x2a,0x2a,0xea,0x0a,0x02,0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x02,0x04,0x07,0x00,},
    /* 21 压 */ {0xe5,0x8e,0x8b,0x00,0x00,0x00,0xff,0x21,0x21,0x21,0xfd,0x21,0xa1,0x21,0x01,0x00,0x04,0x03,0x04,0x04,0x04,0x04,0x07,0x04,0x04,0x05,0x04,0x00,}
    };
    const Font font12x12 = {.w =12, .h = 12, .chars = (const uint8_t *)zh12x12,.len = sizeof(zh12x12)/28, .ascii = &afont12x6};

/* ======================================================== */
/* 以下是手动补全的 font16x16 定义 (修复报错用)            */
/* ======================================================== */

/* * 定义一个空的 16x16 中文字库数组
 * 暂时留空，防止编译报错。如果你以后要显示 16x16 的汉字，
 * 需要用取模工具生成数据填到这里。
 * 每个字符占用 4字节(UTF8) + 32字节(点阵) = 36字节
 */
...
/* * 定义 font16x16 结构体
 * 重点：将 .ascii 指向已存在的 afont16x8
 * 这样即使没有汉字，你也可以用它来显示大号的英文/数字 (16x8大小)
 */
const Font font16x16 = {
    .h = 16,
    .w = 16,
    .chars = (const uint8_t *)zh16x16,
    .len = 0,  // 长度为0，表示目前没有存汉字
    .ascii = &afont16x8 // 关联 ASCII 字库
};